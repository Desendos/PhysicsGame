...
LoadTextures::LoadTextures(char *filename, byte bgcR, byte bgcG, byte bgcB){
	initialise();
	num++;
	LoadGLTexturesAndAlpha(filename, bgcR, bgcG, bgcB);
}
...
//					bufpos = y * bm.bmWidthBytes + x;	//**** v2.0
...
BITMAP LoadTextures::LoadBMPandAlpha(char *Filename, byte bgcR, byte bgcG, byte bgcB){	//Loads a Bitmap & returns bitmap object
	FILE *File=NULL;  // File Handle
	HBITMAP hBmp;
	BITMAP bm;
	...
	if (Filename){
		...
		if (File){							// Does The File Exist?
			...
			unsigned int bSize = bm.bmHeight * bm.bmWidthBytes;  //ensure padding included in width
			textureImageData = new BYTE[bSize*4/3];
			BYTE* tmpBuf = new BYTE[bSize];
			int s = sizeof(bm.bmBits);
			memcpy(tmpBuf, bm.bmBits, bSize);  //copy bmp data to tmp data ptr
			...
			for(int y = 0; y < bm.bmHeight; y++){  //swap back-to-front BGR img to texture RGB img
				for(int x = 0; x < 3 * bm.bmWidth; x += 3){
					newpos = y * 3 * bm.bmWidth + x;     
	//				bufpos = (bm.bmHeight - y - 1) * bm.bmWidthBytes + x;
					bufpos = y * bm.bmWidthBytes + x;	//****
					newpos += c;	//for alpha bit
					textureImageData[newpos + 0] = tmpBuf[bufpos + 2];       
					textureImageData[newpos + 1] = tmpBuf[bufpos+1]; 
					textureImageData[newpos + 2] = tmpBuf[bufpos];     
					textureImageData[newpos + 3] = tmpBuf[bufpos]==bgcB && tmpBuf[bufpos+1]==bgcG && tmpBuf[bufpos+2]==bgcR ? 0 : 255;	//add alpha value for transparency.
					c++;
				}
			}
			delete tmpBuf;
...
bool LoadTextures::LoadGLTexturesAndAlpha(char *s, byte bgcR, byte bgcG, byte bgcB){		// Load Bitmaps And Convert To Textures
	status = false;

// Load The Bitmap, Check For Errors, If Bitmap's Not Found Quit
	glGenTextures(1, texture);		// Create The Textures
	textureImage = LoadBMPandAlpha(s, bgcR, bgcG, bgcB);
	if (textureImage.bmHeight > 0){
		status = true;							// Set The Status To TRUE
		// Typical Texture Generation Using Data From The Bitmap
		glBindTexture(GL_TEXTURE_2D, *texture);
...
// Generate The Texture
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, textureImage.bmWidth, textureImage.bmHeight, 
			0, GL_RGBA, GL_UNSIGNED_BYTE, textureImageData);
	}
